...
class miniProject
{
    private static miniProject.petStats p;
...
double percent;
   ...
   //
   public static void petBirth () throws IOException
   {
       final String name;
       final int bday_day;
       ...
       gameTime(name, songs, artists, links);
...
public static void checkData(int bday_day, int bday_month, int bday_year, String gender, String name) {
   ...
   public static void gameTime(String name, String[] songs, String[] artists, String[] links) throws IOException {
   ...
   for (int i = 0; i < rounds; i++) {
       ...
       // saves score onto text file
       saveScore(totalScore);

       // creates instance of petStats record to be used
       petStats p = new petStats();
   
       // sets the percent 
       p.percent = percent;
   
       checkDeath(p.percent, p);

       // prints highscores from text file
       System.out.println("These are your high scores");
       outputScore();
   
   } // end of gameTime
   
   // checks if the players score is high enough to let the pet live
   public static void checkDeath(double percent, petStats p) {
   
       if (getPercent(p)> 70.0) {
   
           System.out.println("Your pet survives this time.");
       } else {
   
           System.out.println("Your pet has died lolol.");
       }
   }
   
   
   public static double getPercent(petStats p) {
       return p.percent;
   }

   public static void saveScore(double totalScore) throws IOException {
    // Set the second argument of FileWriter to 'true' for append mode
    PrintWriter outputStream = new PrintWriter(new FileWriter("highScores.txt", true));

    outputStream.println(totalScore);
    outputStream.close();
    }

    public static void outputScore() throws IOException {
